  ...
  class SelectComboProductPopupWidget extends Popup {
            ...
            if (self.update_line) {
              ...
              if (order) {
                if (order.get_selected_orderline()) {
                  if (
                    order.get_selected_orderline().product.id ==
    ...
    get req_product() {
      ...
      $.each(this.props.required_products, function (i, prd) {
        prd[
          "product_image_url"
        ] = `/web/image?model=product.product&field=image_128&id=${prd.id}&write_date=${prd.write_date}&unique=1`;
    ...
    getTotalFixedValues() {
      let total = 0;
      this.el.querySelectorAll("span.qty-label").forEach((el) => {
    ...
    AddQty(event) {
      ...
      if (self.getTotalFixedValues() == self.product.optional_limit_qty) {
        self.showPopup("ErrorPopup", {
          title: self.env._t("Limit Exceeded"),
          body: self.env._t("Optional product Limit has exceeded"),
        });
        return;
      }
      ...
      $(".qty-label").each(function (index) {
        ...
        if (currentProductId == prd_id_qty) {
          if (parseInt($(this).text()) > 0) {
            ...
            self.update_optional_product_by_id(
              prd_id_qty,
              parseInt($(this).text())
            );
          }
