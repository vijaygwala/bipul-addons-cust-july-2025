odoo.define("bi_pos_combo.pos", function (require) {
  ...
  const POSComboProduct = (PosGlobalState) =>
    class POSComboProduct extends PosGlobalState {
  ...
  const BiCustomOrderLine = (Orderline) =>
    class BiCustomOrderLine extends Orderline {

       async fetchAndStoreComboProducts(orderLineId) {
      ...
      constructor(obj, options) {
        ...
        this.combo_items_with_qty = this.combo_items_with_qty || [];
        ...
        this.combo_prod_custom_ids = this.combo_prod_custom_ids || [];
         ...
         if (this.combo_products && this.combo_products.length) {
          for (const combo of this.combo_products) {
            this.StoreComboProducts(this.id, combo.id, combo.combo_qty).then(
              () => {
                console.log("Combo products saved for order line");
              }
            );
          }
        }
      ...
      init_from_JSON(json) {
        ...
        this.combo_prod_custom_ids = json.combo_prod_custom_ids || [];

        if (json.combo_items_with_qty) {
          this.combo_items_with_qty =
            json.combo_items_with_qty && JSON.parse(json.combo_items_with_qty);
        }
      ...
      export_as_JSON() {
        ...
        json.combo_products =
          this.combo_products && JSON.stringify(this.combo_products);

        json.combo_items_with_qty =
          this.combo_items_with_qty &&
      ...
      fetchComboProducts(orderLineId) {
        return rpc.query({
          model: "product.combo.custom",
          method: "get_combo_products_by_order_line",
          args: [orderLineId,this.combo_products || null],
      ...
      StoreComboProducts(orderLineId, product_id, qty) {
        return rpc.query({
          model: "pos.order.line",
          method: "store_combo_qty",
          args: [orderLineId, product_id, qty],
        });
      }
      export_for_printing() {
        ...
        // console.log(this.env.pos.get_order().get_orderlines()[0])

        this.fetchComboProducts(this.id).then(function (comboProducts) {
          json.combo_prod_custom_ids = comboProducts;
          console.log("Combo fetched when get method called:", comboProducts);
        });

        return json;
      }

      set_combo_items_with_qty(items) {
        this.combo_items_with_qty = items;
      }

      get_combo_prod_custom_ids() {
        let combos = [];
        this.fetchComboProducts(this.id).then(function (comboProducts) {
          combos = comboProducts;
          console.log("Combo fetched when get method called:", comboProducts);
        });
        // this.set_combo_prod_custom_ids(combos)
        return combos;
      }

      set_combo_prod_custom_ids(ids) {
        this.combo_prod_custom_ids = ids;
      }
