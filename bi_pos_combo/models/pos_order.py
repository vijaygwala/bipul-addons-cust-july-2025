...
class pos_config(models.Model):
    ...
    combo_pack_price = fields.Selection(
        ...
        string='Total Combo Price', default='all_product')
...
class pos_order_line(models.Model):
    ...
    def refund_combo_pro(self):
        pos = self.env['pos.order'].search([('id', '=', self.id)], limit=1)
        for refund in pos.refunded_order_ids:
            for orderline in refund.lines:
...
class PosOrder(models.Model):
    ...
    def refund_order(self,arg):
        pos_order_line = self.env['pos.order.line'].search([('id','=', arg)])
        for orderline in pos_order_line:
            if orderline.combo_prod_ids:
                print("combo prod")
                for product in orderline.combo_prod_ids:
                    stock = self.env['stock.quant'].search(
    ...
    def _get_order_lines(self, orders):
        ...
        for order_line in order_lines:
            ...
            if cstm.combo_prod_custom_ids:
                order_line['combo_prod_custom_ids'] = [(p.id, p.display_name, p.qty) for p in
                                                       cstm.combo_prod_custom_ids]
